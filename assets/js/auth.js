...
const Auth = {
    ...
    async register(username, password) {
        ...
        if (DB.isFirebaseReady()) {
            const remoteUsers = await DB.listUsersRemote().catch(() => []);
            if (remoteUsers.some(u => u && u.username === username)) {
     ...
     * @returns {Promise<{ success: boolean, message: string }>}
     */
    async login(usernameOrUid, password) {
        ...
        // Nếu không tìm thấy trong LocalStorage, thử tìm trực tiếp trên Firebase
        ...
        if (!user && DB.isFirebaseReady()) {
            const remoteUsers = await DB.listUsersRemote().catch(() => []);
            user = remoteUsers.find(u => u && (u.username === usernameOrUid || u.uid === usernameOrUid));
            if (user) DB.saveUsers(remoteUsers);
        }
