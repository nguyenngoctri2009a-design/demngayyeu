const DB = {
    ...
    isFirebaseReady() {
        return typeof window !== 'undefined' && typeof window.FIREBASE_DB_URL === 'string' && window.FIREBASE_DB_URL.length > 0;
    },

    firebaseUrl(path) {
        const base = (window.FIREBASE_DB_URL || '').replace(/\/+$/, '');
    ...
    async firebaseGet(path) {
        ...
        const url = this.firebaseUrl(path);
        const res = await fetch(url, { method: 'GET' });
        const text = await res.text().catch(() => '');
        if (!res.ok) {
            const msg = `Firebase GET ${path} failed (${res.status})`;
            if (typeof showToast === 'function' && (res.status === 401 || res.status === 403)) {
                showToast('Firebase đang chặn truy cập (Rules). Hãy mở quyền đọc/ghi Realtime Database.', 'error');
            }
        ...
        try {
            ...
            if (typeof showToast === 'function') {
                showToast('Firebase trả về dữ liệu không hợp lệ. Kiểm tra `databaseURL` và Rules.', 'error');
            }
            throw new Error(`Firebase GET ${path} returned non-JSON`);
        }
    },

    async firebasePut(path, value) {
        ...
        if (!res.ok) {
            const text = await res.text().catch(() => '');
            const msg = `Firebase PUT ${path} failed (${res.status})`;
            if (typeof showToast === 'function' && (res.status === 401 || res.status === 403)) {
                showToast('Firebase đang chặn truy cập (Rules). Hãy mở quyền đọc/ghi Realtime Database.', 'error');
            }
            throw new Error(`${msg}: ${text}`);
    ...
    async firebasePatch(path, value) {
        if (!this.isFirebaseReady()) return;
        const url = this.firebaseUrl(path);
        const res = await fetch(url, {
            method: 'PATCH',
        ...
        if (!res.ok) {
            const text = await res.text().catch(() => '');
            const msg = `Firebase PATCH ${path} failed (${res.status})`;
            if (typeof showToast === 'function' && (res.status === 401 || res.status === 403)) {
                showToast('Firebase đang chặn truy cập (Rules). Hãy mở quyền đọc/ghi Realtime Database.', 'error');
            }
            throw new Error(`${msg}: ${text}`);
    ...
    async upsertUserRemote(user) {
        if (!this.isFirebaseReady() || !user || !user.uid) return;
        await this.firebasePut(`users/${user.uid}`, user);
    },

    async getUserRemote(uid) {
        if (!this.isFirebaseReady() || !uid) return null;
        return this.firebaseGet(`users/${uid}`);
    },

    async listUsersRemote() {
        if (!this.isFirebaseReady()) return [];
        const data = await this.firebaseGet('users');
        if (!data) return [];
    ...
    saveUsers(users) {
        ...
        if (this.isFirebaseReady() && Array.isArray(users)) {
            users.forEach(u => {
                this.upsertUserRemote(u).catch(() => {});
            });
        }
    },
    ...
    async requestPartner(uid, partnerUid) {
        const me = this.findUserByUID(uid);
        let partner = this.findUserByUID(partnerUid);
        if (!partner && this.isFirebaseReady()) {
            partner = await this.getUserRemote(partnerUid).catch(() => null);
            if (partner && partner.uid) {
                const users = this.getUsers();
                if (!users.some(u => u.uid === partner.uid)) {
                    users.push(partner);
        ...
        this.firebasePut('partner_requests', all).catch(() => {});
        this.firebasePut('partner_outgoing', outgoing).catch(() => {});
    ...
    addUser(user) {
        ...
        users.push(user);
        this.saveUsers(users);
        this.upsertUserRemote(user).catch(() => {});
    },
    ...
    saveUserLocation(uid, lat, lng, address = '') {
        ...
        this.firebasePut('user_locations/' + uid, locations[uid]).catch(() => {});
    },
    ...
    startFirebasePolling() {
        ...
        const pull = async () => {
            try {
                const users = await this.listUsersRemote();
                if (users && users.length >= 0) {
                    localStorage.setItem(this.KEYS.USERS, JSON.stringify(users));
                }
                const req = await this.firebaseGet('partner_requests');
                if (req) localStorage.setItem(this.KEYS.PARTNER_REQUESTS, JSON.stringify(req));
                const out = await this.firebaseGet('partner_outgoing');
                if (out) localStorage.setItem(this.KEYS.PARTNER_OUTGOING, JSON.stringify(out));
                const loc = await this.firebaseGet('user_locations');
                if (loc) localStorage.setItem(this.KEYS.LOCATIONS, JSON.stringify(loc));
            } catch (e) {
            }
        };
